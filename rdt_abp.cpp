...
void A_output(struct msg message) {
    if (A.curstate != WAIT_FOR_LAYER5) {
        printf("  A_output: waiting for ACK for previous message. Dropping the message : %s\n", message.data);
        return;
    }
    ...
    A.curstate = WAIT_FOR_ACK;
    tolayer3(0, packet); // sending packet to layer3
    starttimer(0, INCREMENT);  // starting timer for A
}
...
void A_input(struct pkt packet) {
    if (A.curstate != WAIT_FOR_ACK) {
        printf("  A_input: A is not waiting for acknowledgement. Dropping the packet : %s.\n",packet.payload);
        return;
    }
    ...
    if (packet.acknum != A.seq) {
        printf("  A_input: A is not expecting this packet. Dropping the packet : %s.\n",packet.payload);
        return;
    }
    printf("  A_input: ACK for the  packet : %s.\n",packet.payload);
    stoptimer(0); //stopping the timer.
    A.seq = A.seq^1; //alternating bit
    A.curstate = WAIT_FOR_LAYER5;
}

/* called when A's timer goes off */
void A_timerinterrupt(void) {
    if (A.curstate != WAIT_FOR_ACK) {
        printf("  A_timerinterrupt: this event is only for acknowledge event. Ignoring the event.\n");
        return;
    }
    printf("  A_timerinterrupt: resending the last packet: %s.\n", A.last_packet.payload);
